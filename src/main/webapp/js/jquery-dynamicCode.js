...
function getDaemonArray(paramGroupCode,paramDeviceCode){
	$.ajax({
		...
		success:function(json,textStatus){
			...
			if(paramGroupCode!=0){
				$("#deviceCode").empty().append("<option value='0'>--선택하세요--</option>");
				...
				$("#groupCode").empty().append("<option value='0'>--선택하세요--</option>");
				for(var i=0;i<jsonData.aaData.length;i++){
					var idx = jsonData.aaData[i].idx;
			...
			$("#common_type").empty().append("<option value='0'>--선택하세요--</option>").show();
			$('#mongod_type').hide();
			$('#mongos_type').hide();
		},
