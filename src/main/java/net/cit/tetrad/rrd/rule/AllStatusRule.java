...
@XStreamAlias("AllStatusDatasourceName")
public class AllStatusRule extends BaseRule implements XMLRule {

	private static final long serialVersionUID = 1L;
	private URL url = this.getClass().getResource("/rrd/datasource/all_status_datasource_name.xml");
	...
	public List<StatusDatasourceName> allStatusDatasourceNameXMLToObject() {
		List<StatusDatasourceName> statusDatasourceName = new ArrayList<StatusDatasourceName>();
		
		try {
			AllStatusRule serverStatusRule = (AllStatusRule)BaseRule.fromXML(new File(url.getFile()), AllStatusRule.class);
	...
	public Map<String, StatusDatasourceName> allStatusDatasourceNameByMap() {
		List<StatusDatasourceName> statusInfoList = allStatusDatasourceNameXMLToObject();
